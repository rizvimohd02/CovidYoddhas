...
export const search = (query) => {
  ...
  const trnsctype = query.trnsctype ? `trnsctype=${query.trnsctype}` : ''
  const isBookingMand = query.isBookingMand ? `isBookingMand=${query.isBookingMand}` : ''
  
  return fetch(`${serverUrl}/api/resource?${name}&${place}&${userID}&${trnsctype}&${isBookingMand}`, {
    method: 'GET',
    mode: 'no-cors',
    cache: 'no-cache',
    headers: {
