...
import { update, remove, userID, search } from '../lib/utils'
...
const EditBooking = (props) => {
  const clearItem = { userID: userID(), place: '', name: '', emailid: '', contact: '', person: '1', datetime: '', bookingtime: '7 AM - 8 AM', trnsctype: 'customerBooking', isBookingMand: 'Yes', isDateTimePickerVisible:false }
  ...
  const deleteItem = () => {
    ...
    remove(payload)
      .then(() => {
        Alert.alert('Done', 'Your booking has been cancelled.', [{text: 'OK'}]);
        props.navigation.goBack();
      })
      .catch(err => {
        console.log(err);
...
<ScrollView style={styles.outerView}>
...

      {
        ...
        <TouchableOpacity onPress={updateItem}>
          <Text style={styles.updateButton}>Update Booking</Text>
        </TouchableOpacity>
      }

      <TouchableOpacity onPress={confirmDelete}>
        <Text style={styles.deleteButton}>Cancel Booking</Text>
      </TouchableOpacity>
    </ScrollView>
  );
} else {    
